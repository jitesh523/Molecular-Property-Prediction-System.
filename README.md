...
```json
{
  ...
  "predictions": {"task_1": 0.87},
  "explanation": {"atom_importance": [0.12, 0.44, 0.89, ...]}
}
```
...
│   ├── config.yaml
│   ├── model/                  # gcn.yaml, gat.yaml, mpnn.yaml
│   └── dataset/                # delaney.yaml, bbbp.yaml, ...
├── data/
...
│   ├── models/                 # Baselines, GCN, GAT, MPNN, explainability
│   └── serving/                # FastAPI + model loading
├── tests/                      # pytest test suite
├── results/                    # Generated benchmarks & explanations
...
> **Benchmarked RandomForest and XGBoost baselines against GNN architectures** (GCN, GAT, MPNN) in PyTorch Geometric using scaffold-split cross-validation; reported RMSE/MAE for regression and ROC-AUC/PR-AUC/MCC for imbalanced classification tasks.

> **Delivered production-ready MLOps**: experiment tracking (MLflow), reproducible environments, CI tests, Optuna hyperparameter optimization, and a Dockerized FastAPI inference service for SMILES-to-prediction workflows with explainability artifacts (SHAP + GNNExplainer).
