...
DATASETS = {
    ...
    "sider": {
        ...
        "targets": [
            ...
            "Immune system disorders","Reproductive system & breast disorders",
            "Neoplasms benign malignant & unspecified (incl cysts & polyps)",
            "General disorders & administration site conditions",
            "Endocrine disorders","Surgical & medical procedures",
            "Vascular disorders","Blood & lymphatic system disorders",
            "Skin & subcutaneous tissue disorders",
            "Congenital familial & genetic disorders",
            "Infections & infestations",
            "Respiratory thoracic & mediastinal disorders",
            "Psychiatric disorders","Renal & urinary disorders",
            "Pregnancy puerperium & perinatal conditions",
            "Ear & labyrinth disorders","Cardiac disorders",
...
def download_dataset(name: str, info: dict) -> bool:
    ...
    with open(dest_csv, "w", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=["smiles"] + targets,
                                extrasaction="ignore")
        writer.writeheader()
        valid_rows = 0
        for row in rows:
    ...
    log.info(f"     ✓ {valid_rows} molecules  |  {len(targets)} target(s)  →  {dest_csv.relative_to(ROOT)}")
    return True
